...
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
"""
Run training file for Transformer related networks use.
...
def run(dataset, data_dir, result_dir, num_gpus, total_kimg, gamma,
        ...
        n_samples_per=10, module_list=None, model_type='gan_like',
    ...
    kwargs.update(G_args=G, D_args=D, I_args=I, G_opt_args=G_opt, D_opt_args=D_opt,
                  ...
                  use_info_gan=(model_type == 'info_gan_like'), # Independent branch version
...
def main():
    ...
    parser.add_argument('--model_type', help='Type of model to train', default='ps_sc_like',
                        type=str, metavar='MODEL_TYPE', choices=['gan_like', 'ps_sc_like', 'info_gan_like'])
    ...
    parser.add_argument('--trans_dim', help='Layer dims in transformer.',
                        metavar='TRANS_DIM', default=512, type=int)
    parser.add_argument('--post_trans_wh', help='The cnn h and w after transformer.',
