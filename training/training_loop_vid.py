...
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
"""
Training loop file for video model related networks.
...
def training_schedule(
    ...
    I_lrate_base            = 0.002,    # Learning rate for the MI estimator.
...
def training_loop_vid(
        ...
        I_opt_args={},  # Options for estimator optimizer.
        G_loss_args={},  # Options for generator loss.
        D_loss_args={},  # Options for discriminator loss.
        I_loss_args={},  # Options for estimator loss.
        dataset_args={},  # Options for dataset.load_dataset().
        sched_args={},  # Options for train.TrainingSchedule.
        grid_args={},  # Options for train.setup_snapshot_image_grid().
        metric_arg_list=[],  # Options for MetricGroup.
        tf_config={},  # Options for tflib.init_tf().
        use_vid_head=False,  # Whether to use vc-head.
        use_vid_head_with_cls=False,  # Whether to use classification in discriminator.
        use_vid_naive_cluster=False,  # Whether to use vid clustering.
        use_vid_blurry=False,  # Whether to use vid blurry.
        ...
        I_reg_interval=16,  # How often the perform regularization for I? Ignored if lazy_regularization=False.
        ...
        C_lambda=1,  # Hyper param for VID model.
        MI_lambda=1,  # Hyper param for MINE estimator.
    ...
    for gpu in range(num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            if use_vid_blurry:
                with tf.name_scope('Define_blurry_mem'):
            ...
            if use_vid_head_with_cls:
                if 'lod' in I_info_gpu.vars:
                    lod_assign_ops += [tf.assign(I_info_gpu.vars['lod'], lod_in)]
            with tf.control_dependencies(lod_assign_ops):
                if not use_vid_naive_cluster:
                    with tf.name_scope('G_loss'):
                        if use_vid_head_with_cls:
                            G_loss, G_reg, g_vid_loss, GfromIinfo_loss = dnnlib.util.call_func_by_name(
                                    G=G_gpu, D=D_gpu, I=I_gpu, I_info=I_info_gpu,
                                    opt=G_opt, training_set=training_set, minibatch_size=minibatch_gpu_in,
                                    **G_loss_args)
                        else:
                            G_loss, G_reg, g_vid_loss, _ = dnnlib.util.call_func_by_name(
                                    G=G_gpu, D=D_gpu, I=I_gpu,
                                    opt=G_opt, training_set=training_set, minibatch_size=minibatch_gpu_in,
                                    **G_loss_args)
                    with tf.name_scope('D_loss'):
                        D_loss, D_reg = dnnlib.util.call_func_by_name(
                            G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set,
                    ...
                    with tf.name_scope('I_loss'):
                        if use_vid_blurry:
                            ...
                            I_loss, I_reg = dnnlib.util.call_func_by_name(
                                G=G_gpu, I=I_gpu, opt=I_opt, training_set=training_set,
                                minibatch_size=minibatch_gpu_in, **I_loss_args)
                else:
                    with tf.name_scope('G_loss'):
                        G_loss, G_reg, I_loss, _ = dnnlib.util.call_func_by_name(
                            G=G_gpu, D=D_gpu, I=I_gpu, opt=G_opt,
                            training_set=training_set, minibatch_size=minibatch_gpu_in,
                            **G_loss_args)
                    with tf.name_scope('D_loss'):
                        D_loss, D_reg = dnnlib.util.call_func_by_name(
                            G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set,
            ...
            if not lazy_regularization:
                ...
                if not use_vid_naive_cluster:
                    if I_reg is not None:
                        I_reg_opt.register_gradients(
                            tf.reduce_mean(I_reg * I_reg_interval),
                            I_gpu.trainables)

            if not use_vid_naive_cluster:
                if use_vid_head_with_cls:
                    GIinfo_gpu_trainables = collections.OrderedDict(
                        list(G_gpu.trainables.items()) +
                        list(I_info_gpu.trainables.items())
                    )
                    G_opt.register_gradients(tf.reduce_mean(G_loss + GfromIinfo_loss + g_vid_loss),
                                             GIinfo_gpu_trainables)
                    D_opt.register_gradients(tf.reduce_mean(D_loss),
                                             D_gpu.trainables)
                    I_opt.register_gradients(tf.reduce_mean(I_loss),
                                             I_gpu.trainables)
                else:
                    G_opt.register_gradients(tf.reduce_mean(G_loss + g_vid_loss),
                                             G_gpu.trainables)
                    D_opt.register_gradients(tf.reduce_mean(D_loss),
                                             D_gpu.trainables)
                    I_opt.register_gradients(tf.reduce_mean(I_loss),
    ...
    if use_vid_blurry:
        blurry_assign_op = tf.group(*blurry_assign_ops)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        for _repeat in range(minibatch_repeats):
            ...
            if len(rounds) == 1:
                ...
                if use_vid_blurry:
                    tflib.run([blurry_assign_op], feed_dict)
