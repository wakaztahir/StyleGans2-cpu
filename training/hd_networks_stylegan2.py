...
# --- Last Modified: Wed 22 Apr 2020 23:47:33 AEST
# --- Author: Xinqi Zhu
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
"""
    ...
    for syn_layer_idx in range(dlatent_broadcast):
        with tf.variable_scope('Dis_to_dlatent%d' % syn_layer_idx):
            ...
            with tf.variable_scope('Bottleneck'):
                tmp = apply_bias_act(dense_layer(x[:, start_tmp:end_tmp], fmaps=mapping_fmaps, lrmul=mapping_lrmul),
                                     act=act, lrmul=mapping_lrmul)
            with tf.variable_scope('to_dlatent'):
                x_list.append(apply_bias_act(dense_layer(tmp, fmaps=dlatent_size, lrmul=mapping_lrmul),
                                             act=act, lrmul=mapping_lrmul))
