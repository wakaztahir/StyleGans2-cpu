...
# --- Last Modified: Tue 28 Apr 2020 22:57:33 AEST
# --- Author: Xinqi Zhu
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
"""
...
def build_C_fgroup_layers(x, name, n_latents, start_idx, scope_idx, dlatents_in,
    ...
    with tf.variable_scope(name + '-' + str(scope_idx)):
        ...
        with tf.variable_scope('Att_start_end'):
            ...
            atts = tf.reshape(atts, [-1, att_dim, 1, 1]) # [b*n_latents*2, att_dim, 1, 1]
            att_sm = tf.nn.softmax(atts, axis=1)
            att_cs = tf.cumsum(att_sm, axis=1)
            att_cs_starts, att_cs_ends = tf.split(att_cs, 2, axis=0) # [b*n_latents, att_dim, 1, 1]
...
def build_C_spgroup_layers(x, name, n_latents, start_idx, scope_idx, dlatents_in,
    ...
    with tf.variable_scope(name + '-' + str(scope_idx)):
        ...
        with tf.variable_scope('Att_spatial'):
            ...
            atts_wh = tf.reshape(atts_wh, [-1, x_wh]) # [b*n_latents*4, x_wh]
            att_wh_sm = tf.nn.softmax(atts_wh, axis=1)
            att_wh_cs = tf.cumsum(att_wh_sm, axis=1)
            att_h_cs_starts, att_h_cs_ends, att_w_cs_starts, att_w_cs_ends = tf.split(att_wh_cs, 4, axis=0)
            att_h_cs_ends = 1 - att_h_cs_ends
            att_w_cs_ends = 1 - att_w_cs_ends
            att_h_cs_starts = tf.reshape(att_h_cs_starts, [-1, 1, x_wh, 1])
            att_h_cs_ends = tf.reshape(att_h_cs_ends, [-1, 1, x_wh, 1])
            att_h = att_h_cs_starts * att_h_cs_ends # [b*n_latents, 1, x_wh, 1]
            att_w_cs_starts = tf.reshape(att_w_cs_starts, [-1, 1, 1, x_wh])
            att_w_cs_ends = tf.reshape(att_w_cs_ends, [-1, 1, 1, x_wh])
...
def build_C_spfgroup_layers(x, name, n_latents, start_idx, scope_idx, dlatents_in,
    ...
    with tf.variable_scope(name + '-' + str(scope_idx)):
        ...
        with tf.variable_scope('Att_channel_start_end'):
            ...
            att_sm = tf.nn.softmax(atts, axis=1)
            att_cs = tf.cumsum(att_sm, axis=1)
            att_cs_starts, att_cs_ends = tf.split(att_cs, 2, axis=0)
            att_cs_ends = 1 - att_cs_ends
        ...
        with tf.variable_scope('Att_apply'):
            ...
            att_sp = tf.reshape(att_sp, [-1, n_latents, 1, x_wh, x_wh])
            atts = tf.reshape(atts, [-1, n_latents, att_dim, x_wh, x_wh])
            x_norm = instance_norm(x)
            for i in range(n_latents):
                with tf.variable_scope('style_mod-' + str(i)):
                    x_styled = style_mod(x_norm, C_global_latents[:, i:i+1])
